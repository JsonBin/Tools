...
/// GIF加载数据footer
open class WBRefreshBackGifFooter : WBRefreshBackStateFooter {
    ...
    open override func placeSubviews() {
        ...
        if stateLabel.isHidden {
            ...
            gifView.ve.width = ve.width * 0.5 - labelLeftInset - stateLabel.wb_textWidth * 0.5
        }
    }
    ...
    open override func setState(_ refreshState: WBRefresh.State) {
        ...
        if state == .pulling || state == .refreshing {
            guard let images = stateImages[state] else {
                return
            }
    ...
    private func setImagesForName(_ gifName:String, forState state:WBRefresh.State) -> Void {
        let name = String(format: "com.wbrefresh.autoGifFooter-%08x%08x", arc4random(),arc4random())
